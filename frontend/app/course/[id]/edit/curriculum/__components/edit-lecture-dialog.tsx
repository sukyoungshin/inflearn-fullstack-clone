...
export function EditLectureDialog({isOpen, onClose, lecture}: EditLectureDialogProps) {
  ...
  const {mutate: editLectureMutation, isPending: isEditLecturePending} = useMutation({
      ...
      <DialogContent className='sm:max-w-[700px] max-h-[80vh] overflow-y-auto'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div className='space-y-2'>
            ...
            <CKEditor value={form.description} onChange={value => setForm(prev => ({...prev, description: value}))} />
          </div>
          <div className='flex justify-end space-x-2'>
            <Button variant='outline' type='button' onClick={onClose}>
              취소
            </Button>
            <Button type='submit' disabled={isEditLecturePending}>
