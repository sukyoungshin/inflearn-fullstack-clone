...
export default function UI({initialCourse}: {initialCourse: Course}) {
  ...
  const {data: course} = useQuery<Course>({
    ...
    queryFn: async () => {
      const {data, error} = await api.getCourseById(initialCourse.id);
  ...
  // 강의 추가
  const {mutate: addLectureMutation} = useMutation({
  ...
  // 강의 삭제
  const {mutate: deleteLectureMutation} = useMutation({
  ...
  // 섹션 제목 수정 mutation
  const {mutate: updateSectionTitleMutation} = useMutation({
    mutationFn: async ({sectionId, title}: {sectionId: string; title: string}) => {
  ...
  const handleToggleLecturePreview = (lecture: Lecture) => {
    toggleLecturePreviewMutation(lecture);
  };

  const handleDeleteLecture = (lectureId: string) => {
    deleteLectureMutation({lectureId});
  };
    ...
    <div className='space-y-8 flex flex-col items-center w-full'>
      ...
      <Dialog open={lectureDialogOpen} onOpenChange={setLectureDialogOpen}>
        <DialogContent>
          ...
          />
          <DialogFooter>
            <Button role='button' variant='outline' onClick={() => setLectureDialogOpen(false)}>
              취소
            </Button>
            <Button role='button' onClick={handleAddLecture} variant='default'>
              추가
            </Button>
          </DialogFooter>
        </DialogContent>
