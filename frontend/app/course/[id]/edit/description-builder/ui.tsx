...
export default function UI({course}: {course: Course}) {
  ...
  const {mutate: updateCourseDescriptionMutation, isPending: isUpdateCourseDescriptionPending} = useMutation({
  ...
  return (
    <div className='w-full flex flex-col items-end gap-2'>
      ...
      <Button onClick={() => updateCourseDescriptionMutation()} size='lg' className='font-bold'>
        {isUpdateCourseDescriptionPending ? '저장중...' : '상세소개 저장'}
      </Button>
    </div>
  );
}
