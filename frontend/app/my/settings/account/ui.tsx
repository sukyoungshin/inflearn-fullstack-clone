...
export default function UI({profile}: {profile: User}) {
  ...
  const {mutate: updateProfileMutation, isPending: isUpdateProfilePending} = useMutation({
    mutationFn: async () => {
      ...
      if (!data || error) {
        throw new Error((error as any).message || '프로필 업데이트에 실패했습니다.');
      }
    ...
    <div className='max-w-xl mx-auto bg-white p-8 rounded-lg mt-10 shadow'>
      ...
      <form onSubmit={handleSave} className='space-y-6'>
        ...
        <div>
          ...
          <p className='text-xs text-gray-400 mt-2'>최대 3MB, jpg/png/gif 지원</p>
        </div>
        {/* 이름 */}
        <div>
        ...
        <div className='pt-2'>
          <Button
            ...
            size={'lg'}
          >
            {isUpdateProfilePending ? <Loader2 size={12} className='animate-spin' /> : <span>저장</span>}
          </Button>
        </div>
      </form>
    </div>
